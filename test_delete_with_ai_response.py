...
def test_delete_with_ai_response():
    """测试删除用户消息时同时删除AI回复"""
    base_url = "http://localhost:8000"
    
    print("=" * 60)
    print("测试删除用户消息及其AI回复")
    print("=" * 60)
    
    # 创建测试会话
    session_id = f"delete_ai_test_{int(__import__('time').time())}"
    user_id = "delete_ai_test_user"
    ...
    # 4. 删除第一条用户消息（应该同时删除对应的AI回复）
    print(f"\n4️⃣ 删除第一条用户消息 (ID: {first_message_id})...")
    print("期望：同时删除用户消息和对应的AI回复")
    ...
    # 7. 测试删除AI消息（应该只删除AI消息，不删除用户消息）
    print(f"\n7️⃣ 测试删除AI消息...")
    print("期望：只删除AI消息，不影响用户消息")
    
    # 获取当前的AI消息
    if messages_after:
        ai_messages = [msg for msg in messages_after if msg['role'] == 'assistant']
        if ai_messages:
            ai_message_id = ai_messages[0]['id']
            print(f"尝试删除AI消息 (ID: {ai_message_id})...")
            
            try:
                # 注意：AI消息的user_id可能与用户不同，这里应该会失败
                response = requests.delete(
                    f"{base_url}/chat/messages/{ai_message_id}",
                    params={"user_id": user_id}
                )
                
                if response.status_code == 404:
                    print("✅ 正确阻止了删除AI消息（权限验证通过）")
                elif response.status_code == 200:
                    print("⚠️ AI消息被删除了（可能需要加强权限验证）")
                else:
